...
-- websites können"). gasteig.de liefert bereits ein sauberes og:image über
-- die bestehende Kaskade — diese Pipeline ergänzt nur die bisher fehlende
-- strukturierte Besetzung (event_participants).
alter table events
  add column if not exists gasteig_detail_synced_at timestamptz,
  add column if not exists gasteig_detail_sync_error text;

create index if not exists events_gasteig_detail_pending_idx
  on events (start_datetime)
  where website_url ilike '%gasteig.de/%'
    and gasteig_detail_synced_at is null;

update events
set gasteig_detail_synced_at = null,
...
select cron.schedule('gasteig-detail-sync','*/10 * * * *',$$ select run_gasteig_detail_sync(); $$);

create or replace function queue_near_term_gasteig_resync()
...
select cron.schedule('gasteig-near-term-resync','30 2 * * *',$$ select queue_near_term_gasteig_resync(); $$);
