...
-- BRSO (Nutzeranfrage: "das sollen auch andere websites können").
alter table events
  add column if not exists mphil_detail_synced_at timestamptz,
  add column if not exists mphil_detail_sync_error text;

create index if not exists events_mphil_detail_pending_idx
  on events (start_datetime)
  where website_url ilike '%mphil.de/%'
    and mphil_detail_synced_at is null;

update events
set mphil_detail_synced_at = null,
...
select cron.schedule('mphil-detail-sync','*/10 * * * *',$$ select run_mphil_detail_sync(); $$);

create or replace function queue_near_term_mphil_resync()
...
select cron.schedule('mphil-near-term-resync','25 2 * * *',$$ select queue_near_term_mphil_resync(); $$);
