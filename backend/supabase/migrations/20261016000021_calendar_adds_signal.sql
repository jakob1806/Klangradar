-- Empfehlungssystem-Anfrage (Punkt 7, Recommendation Feedback Loop):
-- "Calendar Adds" fehlte als Signal komplett — der "Zum Kalender
-- hinzufügen"-Button öffnete nur den nativen System-Dialog, ohne dass
-- irgendwo protokolliert wurde, dass ein Nutzer das getan hat. Analog zu
-- ticket_clicks/event_shares: nur für eingeloggte Nutzer (dieselbe
-- Einschränkung wie dort, siehe 20261016000009).
...
create index if not exists idx_calendar_adds_user on calendar_adds(user_id);
create index if not exists idx_calendar_adds_event on calendar_adds(event_id);
alter table calendar_adds enable row level security;
create policy "Nutzer verwaltet eigene Kalender-Hinzufügungen" on calendar_adds
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

insert into home_feed_ranking_weights (key, weight, description) values
  ('calendar_behavior', 2.5, 'Zeitlich gewichtete Kalender-Hinzufügungen ähnlicher Events — stärkstes Intent-Signal, stärker als Ticketklick')
on conflict (key) do update set weight = excluded.weight, description = excluded.description;

-- recommended_events() erneut neu definiert: + Kalender-Signal, analog zu
-- ticket_signal/share_signal in 20261016000015.
