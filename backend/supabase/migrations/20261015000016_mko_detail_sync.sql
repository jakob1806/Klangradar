...
-- auch andere websites können"). m-k-o.eu liefert bereits ein sauberes
-- og:image über die bestehende Kaskade — diese Pipeline ergänzt nur die
-- bisher fehlende strukturierte Besetzung (event_participants).
alter table events
  add column if not exists mko_detail_synced_at timestamptz,
  add column if not exists mko_detail_sync_error text;

create index if not exists events_mko_detail_pending_idx
  on events (start_datetime)
  where website_url ilike '%m-k-o.eu/%'
    and mko_detail_synced_at is null;

update events
set mko_detail_synced_at = null,
...
select cron.schedule('mko-detail-sync','*/10 * * * *',$$ select run_mko_detail_sync(); $$);

create or replace function queue_near_term_mko_resync()
...
select cron.schedule('mko-near-term-resync','35 2 * * *',$$ select queue_near_term_mko_resync(); $$);
