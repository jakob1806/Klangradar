...
-- 20261016000023_festival_follow_signal.sql, discovery_events zuletzt in
-- 20261016000012_recommendation_phase1_signals.sql) — NUR der neue
-- p_city_id-Parameter (München-Default, wie bei den anderen Feed-RPCs)
-- und ein zusätzlicher city_id-Filter im WHERE der Kandidaten-CTE wurden
-- ergänzt. Die komplette Scoring-/Embedding-Logik bleibt unangetastet.
create or replace function recommended_events(
  p_result_limit int default 10,
  p_city_id uuid default munich_city_id()
)
returns table (
...
create or replace function discovery_events(
  p_result_limit int default 10,
  p_city_id uuid default munich_city_id()
)
returns table (
...
grant execute on function discovery_events(int, uuid) to anon, authenticated;
