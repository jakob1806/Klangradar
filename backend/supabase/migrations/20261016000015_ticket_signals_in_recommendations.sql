-- Ticket Intelligence (Nutzeranfrage): "Preis und Verfügbarkeit als
-- zusätzliche Recommendation-Signale verwenden". Zwei neue, unabhängige
-- Signale in recommended_events():
--
-- 1. availability_penalty — ein ausverkauftes Event zu empfehlen ist
--    schlechte UX (kann eh nicht gekauft werden); ein reiner
--    Abendkassen-Hinweis ist nur leichte Reibung, kein harter Malus.
--    "few_left" bleibt bewusst neutral — das ist eher Dringlichkeit
--    (siehe eigenes "Bald ausverkauft"-Modul) als ein Ausschlussgrund.
-- 2. price_affinity — Bonus, wenn price_min in der Preisspanne liegt, in
--    der die Person zuletzt tatsächlich Tickets angeklickt hat (ticket_
--    clicks, dasselbe 180-Tage-Fenster wie ticket_signal). Ohne genug
--    eigene Ticketklick-Historie (< 2) bleibt das Signal neutral (0) statt
--    mit einem einzigen Datenpunkt zu überinterpretieren.
drop function if exists recommended_events(int);

insert into home_feed_ranking_weights (key, weight, description) values
  ('availability_penalty', 6, 'Malus für ausverkaufte Events'),
  ('box_office_penalty', 1.5, 'Malus für Events nur an der Abendkasse'),
  ('price_affinity', 2, 'Bonus wenn der Preis zur bisherigen Ticketklick-Preisspanne der Person passt')
...
with weights as (
  ...
  -- Preisspanne der zuletzt angeklickten Tickets — nur verwertbar ab 2
  -- Datenpunkten, sonst würde ein einziger Klick die ganze Preisspanne
