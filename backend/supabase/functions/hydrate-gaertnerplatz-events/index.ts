...
import { parseGaertnerplatzEventDetail } from "../_shared/gaertnerplatzDetail.ts";
...
// sollen auch andere websites können"). gaertnerplatztheater.de blockt
// normale Fetches nicht. Kein Eventfoto nötig — die Produktionsseite selbst
// hat keins, aber die Bio-Seiten der Mitwirkenden haben saubere Porträts
// (siehe gaertnerplatzDetail.ts), die über die bestehende
// Bildrecherche/website_url-Rückverlinkung gefunden werden.
...
Deno.serve(async (req) => {
  ...
  const { data: events, error } = await supabase.from("events")
    ...
    .ilike("website_url", "%gaertnerplatztheater.de/%")
    .gte("start_datetime", new Date().toISOString())
    .in("status", ["scheduled", "sold_out"])
    .is("gaertnerplatz_detail_synced_at", null)
  ...
  const results = await mapLimit(events ?? [], 3, async (event: any) => {
    try {
      ...
      const detail = parseGaertnerplatzEventDetail(await res.text());
      const participantsAdded = await replaceEventParticipants(supabase, event.id, detail.participants);

      await supabase.from("events").update({
        gaertnerplatz_detail_synced_at: new Date().toISOString(),
      ...
      await supabase.from("events").update({
        gaertnerplatz_detail_synced_at: new Date().toISOString(),
        gaertnerplatz_detail_sync_error: err instanceof Error ? err.message : String(err),
      }).eq("id", event.id);
