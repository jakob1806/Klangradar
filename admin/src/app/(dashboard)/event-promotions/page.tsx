...
export default async function EventPromotionsPage() {
  ...
  return <div className="p-8"><div><h1 className="text-xl font-semibold tracking-tight">Event-Promotions</h1><p className="mt-1 max-w-2xl text-sm text-neutral-500">Prüfe Anträge und gib bei Zustimmung ausschließlich den Zahlungslink frei. Sichtbarkeit startet erst nach bestätigter Stripe-Zahlung; Push-Anfragen werden nie automatisch versendet.</p><p className="mt-2 text-xs text-neutral-500">{items.length} offen</p></div>{error && <p className="mt-6 text-sm text-amber-700">Konnte Promotionen nicht laden: {error.message}</p>}{!error && <div className="mt-6"><PromotionList promotions={items} /></div>}</div>;
}
