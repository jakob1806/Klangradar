...
// Ersetzt den generischen DeleteButton nur auf der Ensemble-Bearbeiten-
// Seite: erst zeigen, was noch verknüpft ist (Veranstaltungen/Quellen/
// Kandidaten), dann erst löschen lassen — Nutzerwunsch nach der Blockier-
// Fehlermeldung: "man soll sehen, was damit verknüpft ist, oder es
// trotzdem löschen dürfen auch wenn dort noch etwas verknüpft ist".

import Link from "next/link";
import { useState, useTransition } from "react";
import { deleteEnsemble, getEnsembleDeleteInfo, type EnsembleDeleteInfo } from "./actions";

type ControlState =
  | { phase: "idle" }
  | { phase: "loading" }
  | { phase: "confirm-simple" }
  | { phase: "confirm-cascade"; info: EnsembleDeleteInfo }
  | { phase: "error"; message: string };

const MAX_LISTED_EVENTS = 8;

export function EnsembleDeleteControl({
  ensembleId,
  ensembleName,
}: {
  ensembleId: string;
  ...
  async function handleStart() {
    ...
    try {
      const info = await getEnsembleDeleteInfo(ensembleId);
      const hasLinks = info.events.length > 0 || info.sources.length > 0 || info.candidateCount > 0;
  ...
  if (state.phase === "confirm-simple") {
    return (
      <span className="inline-flex items-center gap-2 text-sm">
        <span className="text-neutral-600">„{ensembleName}“ wirklich löschen?</span>
        <button
          type="button"
          disabled={pending}
  ...
  return (
    <div className="max-w-sm border-2 border-amber-700 bg-amber-50 p-4 text-sm">
      ...
      {info.events.length > 0 && (
        <div className="mt-2">
          ...
          <ul className="mt-1 flex flex-col gap-0.5">
            {info.events.slice(0, MAX_LISTED_EVENTS).map((e) => (
              <li key={e.id}>
                <Link
                  href={`/events/${e.id}`}
                  target="_blank"
            ...
            {info.events.length > MAX_LISTED_EVENTS && (
              <li className="text-amber-700">… und {info.events.length - MAX_LISTED_EVENTS} weitere</li>
            )}
          </ul>
        </div>
      )}
      ...
      <p className="mt-3 text-xs text-amber-800">
        Beim Löschen verschwindet der Mitwirkenden-Eintrag bei den genannten Veranstaltungen. Datenquellen und
        Entity-Kandidaten bleiben erhalten, verlieren aber die Zuordnung zu diesem Ensemble.
      </p>
