...
export async function createEnsemble(formData: FormData) {
  ...
  const { data: ensembleResolution } = await supabase.rpc("resolve_ensemble_entities", { p_name: f.name });
  const resolvedIds = (ensembleResolution ?? []).map((row: { id: string | null }) => row.id).filter(Boolean);
  if (resolvedIds.length === 1) redirect(`/ensembles/${resolvedIds[0]}`);
  if (resolvedIds.length > 1 || ensembleResolution?.some((row: { resolution: string }) => ["ignore", "ambiguous"].includes(row.resolution))) {
    redirect("/ensemble-families?automatisch=1");
  }
