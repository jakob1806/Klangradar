...
// verknüpft ist (Veranstaltungen/Werke als Komponist/Quellen/Kandidaten),
// dann erst löschen lassen statt zu blockieren.

import Link from "next/link";
import { useState, useTransition } from "react";
import { deletePerson, getPersonDeleteInfo, type PersonDeleteInfo } from "./actions";

type ControlState =
  | { phase: "idle" }
  | { phase: "loading" }
  | { phase: "confirm-simple" }
  | { phase: "confirm-cascade"; info: PersonDeleteInfo }
  | { phase: "error"; message: string };

const MAX_LISTED = 8;

export function PersonDeleteControl({
  personId,
  personName,
}: {
  personId: string;
  ...
  async function handleStart() {
    ...
    try {
      const info = await getPersonDeleteInfo(personId);
      const hasLinks = info.events.length > 0 || info.works.length > 0 || info.sources.length > 0 || info.candidateCount > 0;
  ...
  if (state.phase === "confirm-simple") {
    return (
      <span className="inline-flex items-center gap-2 text-sm">
        <span className="text-neutral-600">„{personName}“ wirklich löschen?</span>
        <button
          type="button"
          disabled={pending}
  ...
  return (
    <div className="max-w-sm rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm">
      <p className="font-medium text-amber-900">„{personName}“ ist noch verknüpft:</p>
      ...
      {info.works.length > 0 && (
        <div className="mt-2">
          <p className="text-xs font-medium uppercase tracking-wide text-amber-700">
            {info.works.length} Werk{info.works.length === 1 ? "" : "e"} als Komponist:in
          </p>
          <ul className="mt-1 flex flex-col gap-0.5">
            {info.works.slice(0, MAX_LISTED).map((w) => (
              <li key={w.id} className="text-amber-800">
                {w.title}
              </li>
            ))}
            {info.works.length > MAX_LISTED && (
              <li className="text-amber-700">… und {info.works.length - MAX_LISTED} weitere</li>
            )}
          </ul>
        </div>
      )}
      ...
      <p className="mt-3 text-xs text-amber-800">
        Beim Löschen verschwindet der Mitwirkenden-Eintrag bei den genannten Veranstaltungen. Werke, Datenquellen und
        Entity-Kandidaten bleiben erhalten, verlieren aber die Zuordnung zu dieser Person.
      </p>
