...
struct EventDetailView: View {
            ...
            EventHeroArtwork(event: event)
                ...
                .overlay(alignment: .top) {
                    HStack(spacing: 10) {
                        ...
                        if let calendarInput = calendarEventInput() {
                            // Nutzerfeedback: statt eines Popups mit
                            // .ics-Downloadmöglichkeit öffnet sich jetzt
                            // direkt Apples nativer Kalender-Editor
                            // (EKEventEditViewController) mit "Hinzufügen"
                            // oben rechts — siehe AddToCalendarView.swift.
                            AddToCalendarButton(calendarInput) {
                                heroButtonLabel(systemImage: "calendar.badge.plus")
                            }
                            .accessibilityLabel("Zum Kalender hinzufügen")
                        }
                        ShareLink(item: shareText) {
                            // Nutzerfeedback: Teilen-Icon wirkte gegenüber
                            // Zurück/Herz/Kalender leicht nach unten versetzt
                            // — "square.and.arrow.up" hat als SF Symbol ein
                            // optisch tiefer sitzendes Glyph-Zentrum als die
                            // anderen drei (der Pfeil ragt oben heraus, das
                            // Tablett sitzt unten). Manueller Ausgleich statt
                            // eines echten Layout-Fehlers.
    ...
    private var titleBlock: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if reportStore.auth.userID != nil {
                // Generated by Claude Code — Nutzervorgabe: der separate
                // "Standort"-Button neben "Als besucht markieren" wird
                // entfernt (zwei unterschiedlich geformte Buttons
                // nebeneinander wirkten uneinheitlich), seine Funktion bleibt
                // aber erhalten — ein Tap auf "Als besucht markieren" prüft
                // jetzt selbst den Standort, sofern eine Spielstätte mit
                // Koordinaten vorliegt, und fragt danach wie zuvor per
                // Dialog nach. Ohne Standortdaten (oder beim Zurücknehmen)
                // wird weiterhin direkt umgeschaltet.
                Button {
                    Task { await markAttended() }
                } label: {
    ...
    /// Generated by Claude Code — ersetzt den früheren separaten
    /// "Standort"-Button: Beim Markieren als besucht wird, sofern eine
    /// Spielstätte mit Koordinaten vorliegt, zuerst der Standort geprüft
    /// (führt bei Übereinstimmung zum Bestätigungsdialog); ohne
    /// Standortdaten oder beim Zurücknehmen wird weiterhin direkt
    /// umgeschaltet.
    @MainActor
    private func markAttended() async {
    ...
    private func toggleAttendance() async {
        ...
        do {
            ...
            // Generated by Claude Code — Check-in ist ein besonderer Moment
            // für Nutzer:innen, entsprechend deutliches Feedback nur beim
            // Markieren als besucht, nicht beim Zurücknehmen.
            if newValue { Haptics.strong() }
        } catch {
            isAttended.toggle()
    ...
    @ViewBuilder private func source(_ value: JSONObject) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            // Generated by Claude Code — Herkunftsangabe der Veranstaltungs-
            // daten (Nutzervorgabe 2026-09-03: "Quellenangaben bei den
            // Konzerten ganz unten, woher es gescraped wurde"). Manuell
            // erfasste Events ohne Quelle zeigen diese Zeile einfach nicht.
            if let sourceName = value.string("_source_name") {
                if let sourceURLString = value.string("_source_url"), let sourceURL = URL(string: sourceURLString) {
                    Link("Veranstaltungsdaten von \(sourceName)", destination: sourceURL)
                } else {
                    Text("Veranstaltungsdaten von \(sourceName)")
                }
            }
        }
        .font(.caption)
    ...
    /// exportieren.
    private func calendarEventInput() -> CalendarEventInput? {
        ...
        return CalendarEventInput(
            title: event.title,
            start: start,
            notes: detail?.string("description_de"),
            location: location,
            url: (detail?.string("ticket_url") ?? detail?.string("website_url")).flatMap(URL.init(string:))
        )
    }
}
