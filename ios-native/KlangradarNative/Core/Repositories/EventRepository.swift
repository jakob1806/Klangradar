...
struct LiveEventRepository: EventRepository {
    ...
    private func enrichingRelatedContent(_ detail: JSONObject) async -> JSONObject {
        ...
        async let sourceAttribution = optionalSourceAttribution(eventID: eventID)
        ...
        if let attribution = await sourceAttribution {
            enriched["_source_name"] = attribution["source_name"] ?? .null
            enriched["_source_url"] = attribution["source_url"] ?? .null
        }
    ...
    /// Generated by Claude Code — Quellenangabe für die Detailansicht
    /// (rechtliche Transparenz-Maßnahme, siehe docs/10-legal-status.md
    /// Punkt 1: die `sources`-Tabelle bleibt admin/editor-only, diese RPC
    /// gibt bewusst nur Name/URL frei). Manuell erfasste Events ohne
    /// `source_id` liefern hier `nil` — kein Fehlerfall.
    private func optionalSourceAttribution(eventID: String) async -> JSONObject? {
        guard !eventID.isEmpty else { return nil }
        let rows: [JSONObject]? = try? await client.rpc("event_source_attribution", parameters: ["p_event_id": .string(eventID)])
        return rows?.first
    }
    ...
    website_url,accessibility,status,image_urls,program_id,source_id,attribution_notice,
